...
/*CH Water pressure*/
unsigned long OpenTherm::buildGetWaterPressureRequest() {
  return buildRequest(OpenThermRequestType::READ, 18, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}

/*Boiler flow temperature*/
unsigned long OpenTherm::buildGetBoilerflowTempRequest() {
  return buildRequest(OpenThermRequestType::READ, 25, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}

/*Relative modulation level*/
unsigned long OpenTherm::buildGetModulationLevelRequest() {
  return buildRequest(OpenThermRequestType::READ, 17, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}

/*Boiler exhaust temperature*/
unsigned long OpenTherm::buildGetExhaustTempRequest() {
  return buildRequest(OpenThermRequestType::READ, OpenThermMessageID::Texhaust, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}

/*Application specific fault flags*/
unsigned long OpenTherm::buildGetAppFaultRequest() {
  return buildRequest(OpenThermRequestType::READ, 5, 0);//OpenThermMessageID::OEMDiagnosticCode, 0);
}
...
/*CH Water pressure*/
float OpenTherm::getCHwaterpressure() {
  unsigned long response = sendRequest(buildGetWaterPressureRequest());
  return getTemperature(response);
}

/*Boiler flow temperature*/
float OpenTherm::getBoilerflowTemp() {
  unsigned long response = sendRequest(buildGetBoilerflowTempRequest());
  return getTemperature(response);
}

/*Relative modulation level*/
float OpenTherm::getModulationLevel() {
  unsigned long response = sendRequest(buildGetModulationLevelRequest());
  return getTemperature(response);
}

/*Boiler exhaust temperature*/
float OpenTherm::getExhaustTemp() {
  unsigned long response = sendRequest(buildGetExhaustTempRequest());
  return response;
}

/*Application specific fault flags*/
float OpenTherm::getAppFault() {
  unsigned long response = sendRequest(buildGetAppFaultRequest());
  return response;
}
